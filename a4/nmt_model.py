...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        # LSTM returns output, (hidden_state, cell_state) VS LSTMCell returns (hidden_state, cell_state)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        X = self.post_embed_cnn(torch.permute(X, (1, 2, 0))) # originally (src_len, b, e) to (b, e, src_len)
        X = torch.permute(X, (2, 0, 1)) # from (b, e, src_len) to (src_len, b, e)
        enc_hiddens, (last_hidden, last_cell) = self.encoder(pack_padded_sequence(X, source_lengths)) # encoder LSTM outputs are: output_tensor, (hidden_state, cell_state)
        enc_hiddens, _ = pad_packed_sequence(enc_hiddens)  
        enc_hiddens = torch.permute(enc_hiddens, (1, 0, 2)) # (src_len, b, h*2) to (b, src_len, h*2)
        last_hidden = torch.cat((last_hidden[0], last_hidden[1]), 1) # from [2, 5, 2] to [5, 4]
        last_cell = torch.cat((last_cell[0], last_cell[1]), 1)
        dec_init_state = (
            self.h_projection(last_hidden),
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        ### YOUR CODE HERE (~9 Lines)
        enc_hiddens_proj = self.att_projection(enc_hiddens) # enc_hiddens = (b, src_len, h*2)
        Y = self.model_embeddings.target(target_padded)
        Y = [y.squeeze(0) for y in torch.split(Y, 1)]
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # "(dec_hidden, dec_cell) = self.decoder(Ybar_t, dec_state)" doesn't work because we need to update the dec_state!!!
        
        e_t = torch.bmm(enc_hiddens_proj, dec_hidden.unsqueeze(2)).squeeze(2) # need to get (b, src_len) by squeezing (b, src_len, 1)
        ...
        a_t = torch.bmm(alpha_t.unsqueeze(1), enc_hiddens).squeeze(1) # bmm returns (b, 1, 2h)
