...
# outside vectors = matrix like 2D array with 10 arrays (each of them are word vectors for separate word in vocab) that are of len 3.
# outside word, center word = len 3 array
...
def naiveSoftmaxLossAndGradient(
    ...
    P_distribution = softmax(dot(outsideVectors, centerWordVec)) # where each element equals P(o|c) of certain o
    loss = - log(P_distribution[outsideWordIdx])
    
    # y = np.zeros(P_distribution.shape)
    # y[outsideWordIdx] = 1     why one-hot

    P_distribution[outsideWordIdx] -= 1

    gradCenterVec = dot(outsideVectors.T, P_distribution) # Sum(P(x|c) for x in all words) - U_o
    gradOutsideVecs = np.outer(P_distribution, centerWordVec) # P(w|c) * V_c (w is all words in vocab)
    # cartesian product?
...
def negSamplingLossAndGradient(
    ...
    theta = dot(outsideVectors[indices], centerWordVec)
    theta[1:] *= -1
    sig = sigmoid(theta)
    loss = - np.sum(log(sig)) # loss = -np.log(sigmoid(np.dot(u_o.T, v_c))) - np.sum(np.log(sigmoid(-np.dot(u_k, v_c))))

    d_theta = 1 - sig
    d_theta[0] *= -1

    gradCenterVec = dot(outsideVectors[indices].T, d_theta) 
    ...
    # naive approach
    # 1. set u_o, u_k, v_c
    # 2. set loss, grads like the annotations above
    

    # derivative of sigmoid = y * (1-y) -> sigmoid(x) * (1 - sigmoid(x))
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for o in outsideWords:
        ...
        loss += gradloss
        gradCenterVecs[centerIdx] += gradCenterVec
